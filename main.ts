...
export default class JapaneseLearningPlugin extends Plugin {
	...
	private createSeparedAnkiDecksBasedOnFileDate = false;
	...
	private async createAnkiCards() {
		...
		if(!this.createSeparedAnkiDecksBasedOnFileDate) {
		...
		for(let file of filesWithTag) {
			...
			if(this.createSeparedAnkiDecksBasedOnFileDate) 
				await Anki.createDeckBasedOnFileDate(sections);
			...
			sections.forEach(section => {
				...
				lines.forEach(async line => {
					if(line.startsWith("# ")) {
						...
						await Anki.createAnkiCard(term, reading, definition);
					}
				})
			})
		}
